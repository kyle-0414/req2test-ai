...
6. 실패 시 원인을 한 문장으로 명확히 설명할 수 있는 수준으로 작성하세요.

[하지 말아야 할 것 (MUST NOT DO)]:
1. 여러 검증 목적을 하나의 TC에 과도하게 묶지 마세요.
2. 기대결과가 일부 step만 설명하는 불완전한 구조로 만들지 마세요.
3. "기능 검증", "전체 확인" 같은 포괄적이고 모호한 제목 사용을 절대 금지합니다.
4. requirement 1개를 그대로 TC 1개로 단순 변환하지 마세요. (연관된 것은 적절히 병합)
5. 반대로, 너무 잘게 분해해 실무 사용성이 떨어지게 만들지 마세요.

필드 작성 가이드:
- scenarioTitle: 15~35자 내외 권장. 구체적인 동작 명시 (예: Info 아이콘 클릭 시 버전 정보 화면 진입 검증)
- scenarioGoal: 제목의 복붙 금지. 이 TC가 무엇을 검증하는지 명확한 문장으로 작성.
- preconditions: 시작 상태, 로그인 여부, 진입 상태 등 테스트 시작 전 필요한 상태만 최소한으로 작성.
- userActions: 사용자가 실제 수행하는 행동 순서대로 구체적으로 작성. ("요구사항에 해당하는 행동을 수행한다" 절대 금지)
- verificationPoints: 화면 이동, 문구/값 표시, 비활성 상태 등 명확하고 구체적인 기대결과 작성.
- automationRecommended: 자동화하기 좋은 시나리오인지 boolean으로 판단 (표시 검증 등은 true, 인간의 정책 주관 개입 시 false).
- relatedRequirementIds: 이 시나리오를 구성하는 데 참조된 원본 req ID 배열.
